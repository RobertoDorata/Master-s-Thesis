...
class BermudanOptionEnv:
    ...
    def __init__(self, data, real_data, risk_free_rate, history_t=15, option_T=30):
        ...
        self.begin_time = 0
        self.stock_index = 0
        self.randomIndexes = [np.random.randint(self.max_num_observations_list[self.stock_index] - self.option_T) for i
                              in range(self.num_episodes)]
        self.random_index = 0
        self.stepBM = 4
    ...
    def _prepare_data(self, data):
        ...
        for stock_data in data:
            ...
            if self.option_T > max_num_observations:
                raise Exception('option_T must be longer than max number of obs. max_num_observations is: {}'.format(
    ...
    def reset(self):
        ...
        self.real_observations = self.real_sequences_list[self.stock_index][
                                 self.begin_time:self.begin_time + self.option_T]
    ...
    def log_parameters(self):
        ...
        # print('max_num_observations: ' + str(self.max_num_observations))
    ...
    def step(self, action):
        ...
        if payoff > 0 and (action == 1 or self.t == (self.option_T - 1)) and self.done == False and (
                self.t >= self.build_warm_up_state_t):
    ...
    def get_discount_factor(self, sell_time):
        time_to_maturity = sell_time / 252.0  # 365.0
    ...
    def getParameters(self, curr_time=None, isReal=False):
        ...
        # gather data from t = 0 to self.t - 1 and calculate mu and sigma
        if self.t == 0:
    ...
    def binomialTreeExercise(self, T, S, K, r, mu, sigma, n, isRiskNeutral):
        #  T... expiration time
        #  S... stock price
        #  K... strike price
        ...
        if isRiskNeutral == False:
            # physical measure
            up = np.exp(mu * deltaT + sigma * np.sqrt(deltaT))
            down = np.exp(mu * deltaT - sigma * np.sqrt(deltaT))
            p_up = 0.5
            p_down = 0.5
        else:
